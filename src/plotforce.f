...
c now turn forces into circular speeds.
      ...
      enddo
 1    continue
cpg 1    call pgbeg(0,'?',1,2)
      write(*,*)
cpg      call pgsch(2.0)
cpg      call pgenv(0.,rplot(nrplot),0.,vmax*1.15,0,0)
cpg      call pgline(nrplot+1,rplot(0),vb(0))
cpg      call pgline(nrplot+1,rplot(0),vd(0))
cpg      call pgline(nrplot+1,rplot(0),vh(0))
cpg      call pgline(nrplot+1,rplot(0),vc(0))
cpg      call pgpt(1,8.,200.,12)
cpg      call pglabel('R [kpc]','Vcirc [km/s]','ROTATION CURVES')
      
cpg      call pgenv(0.,zplot(nzplot),0.,fmax*1.15,0,0)
cpg      call pgline(nzplot+1,zplot(0),fz(0))
      write(toplbl,'(''z-FORCE AT R='',f5.3,'' kpc'')') rsun
cpg      call pglabel('z [kpc]','Kz [(km/s)**2 /pc]',toplbl)
cpg      call pgpt(1,1.1,1.9,12)
cpg      call modstamp
cpg      call pgend
