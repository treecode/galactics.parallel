...
c that, hope that things come together!
     ...
     +          d4rat(0:nrmax)
      ...
      dimension ibuf(100),jbuf(100),kbuf(100),lbuf(4)
     ...
     +                 fszrat(0:nrmax),szrat2(0:nrmax), nrspl
      ...
      read(*,*) niter
      filename='dbh.dat'
      call readharmfile(filename,ibuf,jbuf,kbuf,lbuf)
      eps = dr/1000.
      psi00=pot(eps,0.0)
      rmax = nrdisk*dr
      rfid = 2.5*rdisk
...
c      write(0
c     $     ,'('' ==> Outer dispersion, truncation dispersion:'',2g10.3)'
c     $     ),sqrt(sigr2(outdisk)),drtrunc*fka
