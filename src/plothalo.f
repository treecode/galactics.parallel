...
c      call readdiskdf(filename,ibuf1)      
 ...
 6    continue
      ...
      read(*,*) dbu,nbu
 1    continue
cpg 1    call pgbeg(0,'?',1,1)
      write(*,*)
cpg      call pgvport(0.05,0.32,0.6,0.93)
      call contourden(nin,din,halodens,1.,rscale)
cpg      call pglabel('R [kpc]','z [kpc]','Inner Halo Density')
cpg      call pgvport(0.37,0.64,0.6,0.93)
      call contourden(nbu,dbu,bulgedens,1.,rscale)
cpg      call pglabel('R [kpc]','z [kpc]','Bulge Density')
cpg      call pgvport(0.69,0.96,0.6,0.93)
      call contourden(nout,dout,halodens,1.,rscale)
cpg      call pglabel('R [kpc]','z [kpc]','Outer Halo Density')
cpg      call pgvport(0.05,0.96,0.1,0.42)
      call contourden(nin,din,diskdensf,0.3,rscale)
cpg      call pglabel('R [kpc]','z [kpc]','Disk Density')
      call modstamp
cpg      call pgend
